...
// [Kasraa] Img is used for obtaining the address of applications' functions
// from PIN (i.e., where a function of the program is mapped in the memory
// address space).  Note that 'IMG_AddInstrumentFunction(Img, 0)' inside the
// main() should be uncommented in order for the Img to operate properly.
/*
void Img(IMG img, void*) {
    // [Kasraa] C++ compiler may change the name of functions (i.e., name
    // mangling). Use {extern "C"} for the functions of interest.
    auto rtn = RTN_FindByName(img, "func1");

    if (rtn != RTN_Invalid()) {
        info("The address of the function \'func1\': %lx", RTN_Address(rtn));
    }

    rtn = RTN_FindByName(img, "func2");

    if (rtn != RTN_Invalid()) {
        info("The address of the function \'func2\': %lx", RTN_Address(rtn));
    }
}
*/
